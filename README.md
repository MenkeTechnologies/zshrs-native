...
Or from the tap, once a tag has been pushed:

```sh
brew install MenkeTechnologies/menketech/zshrs-native
```
...
empty checksum. It conflicts with both `zshrs` and `zshrs-all` — all three
install a binary named `zshrs`.
